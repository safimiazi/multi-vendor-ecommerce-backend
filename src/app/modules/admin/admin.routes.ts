...
import { ROLE } from "../../constant/role";
    ...
    router.post("/admin_create_manager", authenticate, authorize(ROLE.ADMIN), adminController.createManager);

    
    router.post("/post_admin", validateRequest(adminPostValidation), adminController.postAdmin);
    router.get("/get_all_admin", adminController.getAllAdmin);
    router.get("/get_single_admin/:id", adminController.getSingleAdmin);
    router.put("/update_admin/:id", validateRequest(adminUpdateValidation), adminController.updateAdmin);
